...
Because expressions are compiled as C# code, they can invoke any method available from:
- The `System` namespace (e.g., `DateTime.Now`, `Guid.NewGuid()`)
- LINQ (`System.Linq`)
- Collections (`System.Collections.Generic`)
- Any assembly loaded into the current AppDomain, including:
  - `System.IO` — file system access
  - `System.Diagnostics` — process spawning
  - `System.Net` — network requests
  - Any custom assemblies you reference
...
1. **Never compile expressions from untrusted sources** (user input, external APIs, uploaded files) without validation.

2. **Validate expressions before compilation.** Use `rule.Validate()` to check syntax, but note that syntax validation does NOT prevent malicious code — it only checks for valid C#.

3. **Restrict available assemblies.** Only load assemblies your expressions legitimately need. Avoid loading `System.IO`, `System.Diagnostics.Process`, etc., if your expressions don't require them.
...
We are exploring:
- Sandboxed compilation with restricted references
- Syntax tree analysis to reject dangerous patterns
- Execution time and memory limits

See issue #13 for assembly reference filtering and related discussions.
